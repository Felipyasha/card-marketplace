...
<template>
  <div class="flex flex-col gap-1.5">
    <label v-if="label" :for="id" class="font-display font-bold text-sm tracking-wider text-zinc-400">
      {{ label }}
    </label>

    <input v-bind="$attrs" :id="id" v-model="model" :aria-invalid="!!error"
      :aria-describedby="error ? `${id}-error` : hint ? `${id}-hint` : undefined" :class="[
