...
<template>
  <div>
    ...
    <div v-if="tradesStore.loading && tradesStore.trades.length === 0"
    ...
    <div v-else-if="tradesStore.trades.length === 0" class="flex flex-col items-center justify-center py-24 gap-4">
    ...
    <div v-else class="flex flex-col gap-4">
      <TradeCard v-for="trade in tradesStore.trades" :key="trade.id" :trade="trade" />


      <div v-if="tradesStore.more" class="flex justify-center mt-4">
        <AppButton variant="ghost" :loading="tradesStore.loading" @click="tradesStore.fetchMoreTrades()">
          Carregar mais
        </AppButton>
      </div>
    </div>
